...
# Assumes the dir to contain two folders, simulation_xskewed and simulation_yskewed in which the results will be saved
# Simulation is quite time consuming, therefor intermediate saving is recommended
...
run_simulation = function(train_size, runs = 10, epsilon = 1, scenario, skewness = 0){
  ...
  for (run in 1:runs) {
    ...
    if (scenario == "skewed_x"){
      train_population = simulate_data_skewed(N = N, p = 5, p_0 = 5, epsilon = epsilon, skewness = skewness)
      test_data = simulate_data_skewed(N = 1000, p = 5, p_0 = 5, epsilon = epsilon, skewness = skewness)
    } else {
  ...
  if (scenario=="skewed_x"){
    write.csv2(outframe, paste0("simulation_xskewed/simulation n =", train_size, ", skewness = ", skewness, ".csv"))
  } else {
    write.csv2(outframe, paste0("simulation_yskewed/simulation n =", train_size, ", skewness = ", skewness, ".csv"))
  }
}
